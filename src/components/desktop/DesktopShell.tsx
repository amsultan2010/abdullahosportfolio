...
function NowPlaying() {
  ...
  useEffect(() => {
    const fetchTrack = () => {
      fetch('/api/spotify')
        .then(r => r.ok ? r.json() : null)
        .then(d => { if (d && d.title) { setTrack(d); setProgress(d.progressMs || 0); } })
        .catch(() => {});
    };
    fetchTrack();
  ...
  useEffect(() => {
    if (!track) return;
    // Tick progress for live tracks, or simulate for playlist fallback
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flex: 1 }}>
        {track.albumArt ? (
          ...
          <div style={{ width: 48, height: 48, borderRadius: 6, flexShrink: 0, background: 'rgba(29, 185, 84, 0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
